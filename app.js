...
mongoose.connect("mongodb://localhost:27017/dailyjournalDB", {useNewUrlParser: true , useUnifiedTopology: true });
//mongoose.connect("mongodb://localhost:27017/dailyjournalDB", { useNewUrlParser: true , useNewUrlParser: true , useUnifiedTopology: true });
...
const PostDb = mongoose.model("postdb",postDbSchema);
...
app.get("/",function(req,res){
  PostDb.find({},function(err,result){
    if(!err){
...
PostDb.findOne({_id:requestedPostId},function(err,post){
  if(err){
    ...
    //result.forEach(function(post){
      ...
      // if(requestedPostId === post._id.toString()){
      // console.log(requestedPostId +" // "+ post._id );
      // }
      //if(lodash.lowerCase(post.title)===lodash.lowerCase(req.params.postName) ){
          res.render("post", {postNameEjs : post.title , contentEjs : post.content})
      //}
    // });
  }
});
